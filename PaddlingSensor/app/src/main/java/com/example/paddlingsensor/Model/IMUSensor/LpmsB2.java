...
public class LpmsB2 extends Thread {

    //00001101-0000-1000-8000-00805f9b34fb = UUID for IMU-sensor
    ...
    int frameCounter = 0;
    // Data Logging
    private IMUDataLogger dataLogger;
    private String node;
    private Context context;
    private PaddlingSensorModel model;
    private IMUQueue<IMUQueueModel> queue;
    private long receivingTime;


    public LpmsB2(String node, Context context, PaddlingSensorModel model, IMUQueue<IMUQueueModel> queue) {
        this.node = node;
        this.context = context;
        this.model = model;
        mAdapter = BluetoothAdapter.getDefaultAdapter();
        dataLogger = new IMUDataLogger(node, context);
        dataLogger.CreateFile();
        this.queue = queue;
        receivingTime = 0;
    }
    ...
    public void setStreamFrequency(final int freq) {
        ...
        if (freq == LPMS_STREAM_FREQ_5HZ ||
            ...
            try {
                ...
                System.out.println("Error joining thread when setting frequency");
                e.printStackTrace();
            }
        }
    }
    ...
    /**
     * @return Return the data length of the sensor.
     */
...
//        }
    ...
    /**
     * Add sensor data to its corresponding queue
     */
    ...
    /**
     * Concatenate sensor data to its file
     */
    private void concatenateToFile() {
        dataLogger.LogNewLine(mLpmsBData.acc[0] + "," + mLpmsBData.acc[1] + "," + mLpmsBData.acc[2] + "," +
                mLpmsBData.gyr[0] + "," + mLpmsBData.gyr[1] + "," + mLpmsBData.gyr[2] + "," + mLpmsBData.euler[0] + "," + receivingTime);
    }
    ...
    private void updateUI() {
        model.handleSensorData(mLpmsBData.acc[0], 1);
        model.handleSensorData(mLpmsBData.acc[1], 2);
        model.handleSensorData(mLpmsBData.acc[2], 3);
//        model.handleSensorData(mLpmsBData.gyr[0], 4);
//        model.handleSensorData(mLpmsBData.gyr[1], 5);
//        model.handleSensorData(mLpmsBData.gyr[2], 6);
        model.handleSensorData(mLpmsBData.euler[0], 4);
        model.handleSensorData(mLpmsBData.euler[1], 5);
        model.handleSensorData(mLpmsBData.euler[2], 6);
    }
